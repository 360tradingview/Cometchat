...
jqcc.extend($.mobilewebapp, {
        ...
        $.each(item, function(i, incoming){
            ...
            if(incoming.message.indexOf('CC^CONTROL_') != -1){
                ...
                if (controlparameters.method == 'kicked' || controlparameters.method == 'banned') {
                    if(jqcc.cometchat.getChatroomVars('myid')==controlparameters.params.id){
                        if(controlparameters.method=='kicked'){
                            jqcc.mobilewebapp.kickUser(controlparameters.method, incoming.id);
                        ...
                        if(controlparameters.method=='banned'){
                            jqcc.mobilewebapp.banUser(controlparameters.method, incoming.id);
    ...
    mobilecreateChatroomSubmit: function(){
        ...
        jqcc('#createChatroomForm').find('#name').val('');
        jqcc('#createChatroomForm').find('#password').val('');
        jqcc.mobilewebapp.chatHeartbeatWebapp(1);
    },
